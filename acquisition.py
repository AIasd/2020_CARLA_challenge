...
class Acquirer:
    ...
    def select_batch(self, model, pool_data, unique_len=0, uncertainty_conf=False):
        ...
        for batch_idx, (data, _) in enumerate(pool_loader):
            ...
            scores[batch_idx:end_idx] = self.score(model, data.to(self.device).float(), uncertainty_conf=uncertainty_conf)
...
class BatchBALD(Acquirer):
    ...
    def select_batch(self, model, pool_data, k=100, unique_len=0, uncertainty_conf=False):
        # I(y;W | x) = H1 - H2 = H(y|x) - E_w[H(y|x,W)]
        ...
        if num_extra > 0:
            ...
            sub_pool_data = torch.utils.data.Subset(pool_data, np.arange(self.num_sub_pool))
        else:
            ...
            sub_pool_data = torch.utils.data.Subset(pool_data, np.arange(len(pool_data)))
        ...
        with torch.no_grad():
            ...
            for batch_idx, (data, _) in enumerate(pool_loader):
                ...
                torch.manual_seed(0)
                pool_p_y[batch_idx:end_idx] = torch.stack([model.predict_proba(data.to(self.device).float()) for i in range(k)], dim=1).permute(0,2,1)
        ...
        if uncertainty_conf:
            s_conf = super(BatchBALD, BatchBALD).calculate_conf(model, data_list.to(self.device).float())
    ...
    def select_batch(self, model, pool_data, unique_len=0, uncertainty_conf=False):
        ...
        if unique_len > 0:
            if unique_len < self.batch_size:
                ...
                pool_data = torch.utils.data.Subset(original_pool_data, np.arange(len(unique_len), len(pool_data)))

            elif unique_len < len(pool_data):
                pool_data = torch.utils.data.Subset(original_pool_data, np.arange(len(unique_len)))
        ...
        # print('s_conf', s_conf)
        cur_inds = init_centers(grad_np, s_conf, self.batch_size)
        # if unique_len > 0:
        #     scores[:unique_len] += torch.max(scores)
...
def map_acquisition(acquisition_name):
    uncertainty_map = {'BUGCONF':BUGCONF, 'Random':Random, 'BALD':BALD, 'BatchBALD':BatchBALD, 'BADGE':BADGE}
